...
for (const c of Object.keys(promisifiables)) {
  for (const m of Object.keys(promisifiables[c])) {
    gdal[c].prototype[m] = (function () {
      const promisified = promisify(gdal[c].prototype[m])
      const original = gdal[c].prototype[m]
      const cbArg = promisifiables[c][m]
      return function () {
        let callback
