 ...
 * If the last parameter is a callback, then this callback is called on completion and a resolved Promise is returned. Otherwise the function returns a Promise resolved with the result.
 *
 * @example
 * ```
 ...
 * @param {Integer} [data_type] Used when creating a raster dataset with the `"w"` mode.
 * @param {String[]|object} [creation_options] Used when creating a dataset with the `"w"` mode.
 * @param {requestCallback} [callback] Standard callback, always the last parameter
 *
 * @return {gdal.Dataset}
 */
...
gdal.RasterBandPixels.prototype.readAsync = (function () {
  ...
  return async function (x, y, width, height, data, options, cb) {
