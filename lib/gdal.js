...
if (process.env.PROJ_LIB === undefined && !fs.existsSync(proj_path)) {
  throw new Error(
    `The bundled proj data path for node-gdal is missing '${proj_path}' and PROJ_LIB environment is not set`
  )
}

// needs to be set before the binding is loaded
if (process.env.PROJ_LIB === undefined) {
  process.env.PROJ_LIB = proj_path
}
...
gdal.openAsync = (function () {
  const openAsync = gdal.openAsync
  ...
  return async function (
    ...
    if (mode === 'w') {
      ...
      const args = Array.prototype.slice.call(arguments, 3) // x_size, y_size, ...
      args.unshift(filename)
      return driver.createAsync.apply(driver, args)
    }
    ...
    if (arguments.length > 2 && drivers) {
      ...
      for (const driver_name of drivers) {
        ...
        ds = await driver.openAsync(filename, mode).catch(() => undefined)
        if (ds) break
      }
    ...
    // call gdal.open() method normally
    return openAsync.call(gdal, filename, mode, callback)
  }
})()


const promisify = require('util').promisify
gdal.openPromise = (function () {
  const openPromise = promisify(gdal.openAsync)
  return function (path, mode, drivers, x_size, y_size,
    n_bands, datatype, options) {
    return openPromise(path, mode, drivers, x_size, y_size,
      n_bands, datatype, options)
  }
})()
gdal.RasterBandPixels.prototype.readPromise = (function () {
  ...
  return function (x, y, width, height, data, options) {
    ...
    return readPromise.call(this, x,
      y,
      width,
      height,
      data,
...
gdal.RasterBandPixels.prototype.writePromise = (function () {
  const writePromise = promisify(gdal.RasterBandPixels.prototype.writeAsync)
  return function (x, y, width, height, data, options) {
    if (!options) options = {}
    if (data) data._gdal_type = getTypedArrayType(data)
    console.log(arguments)
    return writePromise.call(this, x,
      y,
      width,
      height,
      data,
...
gdal.RasterBandPixels.prototype.readAsync = (function () {
  const read = gdal.RasterBandPixels.prototype.readAsync
...
gdal.RasterBandPixels.prototype.writeAsync = (function () {
  const writeAsync = gdal.RasterBandPixels.prototype.writeAsync
  return function (x, y, width, height, data, options, cb) {
    if (!options) options = {}
    if (data) data._gdal_type = getTypedArrayType(data)
    console.log(arguments)
    return writeAsync.apply(this, [
      x,
      y,
      width,
      height,
