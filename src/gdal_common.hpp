...
#define ATTR(t, name, get, set)                                                \
  Nan::SetAccessor(                                                            \
    t->InstanceTemplate(), Nan::New(name).ToLocalChecked(), get, set);
...
#define NODE_WRAPPED_FROM_OBJ(obj, key, type, var)                             \
  {                                                                            \
    ...
    Local<Value> val = Nan::Get(obj, sym).ToLocalChecked();                    \
    if (                                                                       \
      !val->IsObject() || val->IsNull() ||                                     \
      !Nan::New(type::constructor)->HasInstance(val)) {                        \
...
#define NODE_ARG_ARRAY(num, name, var)                                         \
  ...
  if (!info[num]->IsArray()) {                                                 \
    return Nan::ThrowTypeError(                                                \
      (std::string(name) + " must be an array").c_str());                      \
  }                                                                            \
...
#define NODE_ARG_OBJECT(num, name, var)                                        \
  ...
  if (!info[num]->IsObject()) {                                                \
    return Nan::ThrowTypeError(                                                \
      (std::string(name) + " must be an object").c_str());                     \
  }                                                                            \
...
#define NODE_ARG_WRAPPED(num, name, type, var)                                 \
  ...
  if (                                                                         \
    info[num]->IsNull() || info[num]->IsUndefined() ||                         \
    !Nan::New(type::constructor)->HasInstance(info[num])) {                    \
...
#define NODE_ARG_WRAPPED_OPT(num, name, type, var)                             \
  if (                                                                         \
    info.Length() > num && !info[num]->IsNull() &&                             \
    !info[num]->IsUndefined()) {                                               \
...
#define NODE_WRAPPED_METHOD_WITH_RESULT(                                       \
  klass, method, result_type, wrapped_method)                                  \
  NAN_METHOD(klass::method) {                                                  \
    ...
    info.GetReturnValue().Set(                                                 \
      Nan::New<result_type>(obj->this_->wrapped_method()));                    \
  }
...
#define NODE_WRAPPED_METHOD_WITH_RESULT_1_WRAPPED_PARAM(                       \
  ...
  NAN_METHOD(klass::method) {                                                  \
    ...
    klass *obj = Nan::ObjectWrap::Unwrap<klass>(info.This());                  \
    if (!obj->isAlive())                                                       \
      return Nan::ThrowError(#klass " object has already been destroyed");     \
    info.GetReturnValue().Set(                                                 \
      Nan::New<result_type>(obj->this_->wrapped_method(param->get())));        \
  }
...
#define NODE_WRAPPED_METHOD_WITH_RESULT_1_ENUM_PARAM(                          \
  ...
  NAN_METHOD(klass::method) {                                                  \
    ...
    info.GetReturnValue().Set(                                                 \
      Nan::New<result_type>(obj->this_->wrapped_method(param)));               \
  }

#define NODE_WRAPPED_METHOD_WITH_RESULT_1_STRING_PARAM(                        \
  klass, method, result_type, wrapped_method, param_name)                      \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    std::string      param;                                                    \
    ...
    info.GetReturnValue().Set(                                                 \
      Nan::New<result_type>(obj->this_->wrapped_method(param.c_str())));       \
  }

#define NODE_WRAPPED_METHOD_WITH_RESULT_1_INTEGER_PARAM(                       \
  klass, method, result_type, wrapped_method, param_name)                      \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    int              param;                                                    \
...
#define NODE_WRAPPED_METHOD_WITH_RESULT_1_DOUBLE_PARAM(                        \
  klass, method, result_type, wrapped_method, param_name)                      \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    double           param;                                                    \
...
#define NODE_WRAPPED_METHOD_WITH_CPLERR_RESULT_1_WRAPPED_PARAM(                \
  klass, method, wrapped_method, param_type, param_name)                       \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    param_type *     param;                                                    \
    NODE_ARG_WRAPPED(0, #param_name, param_type, param);                       \
...
#define NODE_WRAPPED_METHOD_WITH_CPLERR_RESULT_1_STRING_PARAM(                 \
  klass, method, wrapped_method, param_name)                                   \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    std::string      param;                                                    \
    NODE_ARG_STR(0, #param_name, param);                                       \
...
#define NODE_WRAPPED_METHOD_WITH_CPLERR_RESULT_1_INTEGER_PARAM(                \
  klass, method, wrapped_method, param_name)                                   \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    int              param;                                                    \
...
#define NODE_WRAPPED_METHOD_WITH_CPLERR_RESULT_1_DOUBLE_PARAM(                 \
  klass, method, wrapped_method, param_name)                                   \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    double           param;                                                    \
...
#define NODE_WRAPPED_METHOD_WITH_OGRERR_RESULT_1_WRAPPED_PARAM(                \
  klass, method, wrapped_method, param_type, param_name)                       \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    param_type *     param;                                                    \
...
#define NODE_WRAPPED_METHOD_WITH_OGRERR_RESULT_1_STRING_PARAM(                 \
  klass, method, wrapped_method, param_name)                                   \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    std::string      param;                                                    \
...
#define NODE_WRAPPED_METHOD_WITH_OGRERR_RESULT_1_INTEGER_PARAM(                \
  klass, method, wrapped_method, param_name)                                   \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    int              param;                                                    \
...
#define NODE_WRAPPED_METHOD_WITH_OGRERR_RESULT_1_DOUBLE_PARAM(                 \
  klass, method, wrapped_method, param_name)                                   \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    double           param;                                                    \
...
#define NODE_WRAPPED_METHOD_WITH_1_WRAPPED_PARAM(                              \
  klass, method, wrapped_method, param_type, param_name)                       \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    param_type *     param;                                                    \
...
#define NODE_WRAPPED_METHOD_WITH_1_INTEGER_PARAM(                              \
  klass, method, wrapped_method, param_name)                                   \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    int              param;                                                    \
...
#define NODE_WRAPPED_METHOD_WITH_1_DOUBLE_PARAM(                               \
  klass, method, wrapped_method, param_name)                                   \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    double           param;                                                    \
...
#define NODE_WRAPPED_METHOD_WITH_1_BOOLEAN_PARAM(                              \
  klass, method, wrapped_method, param_name)                                   \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    bool             param;                                                    \
...
#define NODE_WRAPPED_METHOD_WITH_1_ENUM_PARAM(                                 \
  klass, method, wrapped_method, enum_type, param_name)                        \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    enum_type        param;                                                    \
    NODE_ARG_ENUM(0, #param_name, enum_type, param);                           \
...
#define NODE_WRAPPED_METHOD_WITH_1_STRING_PARAM(                               \
  klass, method, wrapped_method, param_name)                                   \
  NAN_METHOD(klass::method) {                                                  \
    Nan::HandleScope scope;                                                    \
    std::string      param;                                                    \
