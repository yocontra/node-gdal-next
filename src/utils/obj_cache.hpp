...
template <typename K> struct ObjectCacheItem {
...
// objects are removed from cache and their destructor is called when they go
// out of scope

template <typename K, typename W> class ObjectCache {
    public:
...
template <typename K, typename W> ObjectCache<K, W>::ObjectCache() : cache(), aliases() {
}

template <typename K, typename W> ObjectCache<K, W>::~ObjectCache() {
}
...
void ObjectCache<K, W>::release(ObjectCacheItem<K> *item, ObjectCacheItemStatus status_change) {
  ...
  if (item->status == ITEM_RELEASED_FROM_ALL) { delete item; }
}

template <typename K, typename W> void ObjectCache<K, W>::add(K *key, K *alias, v8::Local<v8::Object> obj) {
...
template <typename K, typename W> void ObjectCache<K, W>::add(K *key, v8::Local<v8::Object> obj) {
  add(key, NULL, obj);
}

template <typename K, typename W> bool ObjectCache<K, W>::has(K *key) {
  return cache.count(key) > 0 || aliases.count(key) > 0;
};

template <typename K, typename W> v8::Local<v8::Object> ObjectCache<K, W>::get(K *key) {
  Nan::EscapableHandleScope scope;
  return scope.Escape(Nan::New(getItem(key)->obj));
}

template <typename K, typename W> W *ObjectCache<K, W>::getWrapped(K *key) {

  ObjectCacheItem<K> *item = getItem(key);
  v8::Local<v8::Object> obj = Nan::New(item->obj);

  bool is_near_death = item->obj.IsNearDeath();
  if (is_near_death) { LOG("About to Unwrap Near Death handle [%p]", key); }

  W *unwrapped = Nan::ObjectWrap::Unwrap<W>(obj);

  if (is_near_death) { LOG("Unwrapped Near Death handle [%p]", key); }
  return unwrapped;
}

template <typename K, typename W> ObjectCacheItem<K> *ObjectCache<K, W>::getItem(K *key) {
  // return handle to existing object if already wrapped
  // check by calling has() first
  if (aliases.count(key) > 0) { key = aliases[key]; }
  return cache[key];
}

template <typename K, typename W> void ObjectCache<K, W>::erase(K *key) {
  if (has(key)) { erase(getItem(key)); }
}

template <typename K, typename W> void ObjectCache<K, W>::erase(ObjectCacheItem<K> *item) {
  K *key = (K *)item->key;
  cache.erase(key);
  if (item->alias) { aliases.erase(item->alias); }
