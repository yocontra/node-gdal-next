...
void GeometryCollection::Initialize(Local<Object> target) {
  ...
  Local<FunctionTemplate> lcons =
    Nan::New<FunctionTemplate>(GeometryCollection::New);
  ...
  Nan::Set(
    target,
    Nan::New("GeometryCollection").ToLocalChecked(),
...
NAN_METHOD(GeometryCollection::New) {
  ...
  if (info[0]->IsExternal()) {
    ...
    if (info.Length() != 0) {
      Nan::ThrowError(
        "GeometryCollection constructor doesn't take any arguments");
      return;
    }
...
NODE_WRAPPED_METHOD_WITH_RESULT(
  GeometryCollection, getLength, Number, get_Length);
...
NAN_GETTER(GeometryCollection::childrenGetter) {
  ...
  info.GetReturnValue().Set(
    Nan::GetPrivate(info.This(), Nan::New("children_").ToLocalChecked())
      .ToLocalChecked());
}
