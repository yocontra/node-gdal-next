...
void FeatureFields::Initialize(Local<Object> target) {
  ...
  Local<FunctionTemplate> lcons =
    Nan::New<FunctionTemplate>(FeatureFields::New);
  ...
  Nan::Set(
    target,
    Nan::New("FeatureFields").ToLocalChecked(),
...
Local<Value> FeatureFields::New(Local<Value> layer_obj) {
  ...
  v8::Local<v8::Object> obj =
    Nan::NewInstance(
      Nan::GetFunction(Nan::New(FeatureFields::constructor)).ToLocalChecked(),
      1,
      &ext)
      .ToLocalChecked();
  ...
  if (info.Length() == 1) {
    if (info[0]->IsArray()) {
      ...
      for (i = 0; i < n; i++) {
        ...
        if (
          field_index == -1 ||
          !Nan::HasOwnProperty(values, Nan::New(field_name).ToLocalChecked())
             .FromMaybe(false)) {
          continue;
        }

        Local<Value> val =
          Nan::Get(values, Nan::New(field_name).ToLocalChecked())
            .ToLocalChecked();
...
NAN_METHOD(FeatureFields::reset) {
  ...
  for (i = 0; i < n; i++) {
    ...
    Local<Value> val =
      Nan::Get(values, Nan::New(field_name).ToLocalChecked()).ToLocalChecked();
    if (setField(f->get(), field_index, val)) {
      Nan::ThrowError("Unsupported type of field value");
      return;
    }
...
NAN_METHOD(FeatureFields::indexOf) {
  ...
  info.GetReturnValue().Set(
    Nan::New<Integer>(f->get()->GetFieldIndex(name.c_str())));
}
...
Local<Value> FeatureFields::get(OGRFeature *f, int field_index) {
  ...
  switch (field_def->GetType()) {
    ...
    case OFTInteger64:
      return scope.Escape(
        Nan::New<Number>(f->GetFieldAsInteger64(field_index)));
#endif
    case OFTReal:
...
Local<Value>
FeatureFields::getFieldAsIntegerList(OGRFeature *feature, int field_index) {
  Nan::EscapableHandleScope scope;

  int count_of_values = 0;

  const int *values =
    feature->GetFieldAsIntegerList(field_index, &count_of_values);
...
Local<Value>
FeatureFields::getFieldAsDoubleList(OGRFeature *feature, int field_index) {
  Nan::EscapableHandleScope scope;

  int count_of_values = 0;

  const double *values =
    feature->GetFieldAsDoubleList(field_index, &count_of_values);
...
Local<Value>
FeatureFields::getFieldAsStringList(OGRFeature *feature, int field_index) {
  Nan::EscapableHandleScope scope;
...
Local<Value>
FeatureFields::getFieldAsBinary(OGRFeature *feature, int field_index) {
  Nan::EscapableHandleScope scope;

  int count_of_bytes = 0;

  unsigned char *data =
    (unsigned char *)feature->GetFieldAsBinary(field_index, &count_of_bytes);
...
Local<Value>
FeatureFields::getFieldAsDateTime(OGRFeature *feature, int field_index) {
  Nan::EscapableHandleScope scope;
  ...
  int result = feature->GetFieldAsDateTime(
    field_index, &year, &month, &day, &hour, &minute, &second, &timezone);
  ...
  if (result == TRUE) {
    ...
    if (year) {
      Nan::Set(
        hash, Nan::New("year").ToLocalChecked(), Nan::New<Integer>(year));
    }
    if (month) {
      Nan::Set(
        hash, Nan::New("month").ToLocalChecked(), Nan::New<Integer>(month));
    }
    if (day) {
    ...
    if (hour) {
      Nan::Set(
        hash, Nan::New("hour").ToLocalChecked(), Nan::New<Integer>(hour));
    }
    if (minute) {
      Nan::Set(
        hash, Nan::New("minute").ToLocalChecked(), Nan::New<Integer>(minute));
    }
    if (second) {
      Nan::Set(
        hash, Nan::New("second").ToLocalChecked(), Nan::New<Integer>(second));
    }
    if (timezone) {
      Nan::Set(
        hash,
        Nan::New("timezone").ToLocalChecked(),
        Nan::New<Integer>(timezone));
    }
